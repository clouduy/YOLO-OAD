...
class Conv_DCNv4(nn.Module):
    ...
    def forward(self, x):
        N, C, H, W = x.shape
        L = H * W
        x = x.permute(0, 2, 3, 1).reshape(N, L, C).contiguous()
        x = self.dcnv4(x)
        x = x.view(N, H, W, -1).permute(0, 3, 1, 2).contiguous()
        x = self.conv(x)
...
class LADH(nn.Module):
    def __init__(self, ch=256, nc=20, anchors=(), e_lambda=1e-4):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors

        # 初始化一个共享的 Conv_DCNv4 层
        self.shared_conv_dcnv4 = Conv_DCNv4(ch, ch, k=1, s=1, p=None, g=1, d=1, act=True)

        # 所有分支在 Conv_DCNv4 层之后
        self.cls_conv = DWConv(ch, 256, 3, 1)
        # self.cls_conv = XSepConv(ch, 256, 3, 1, padding=1)
        self.iou_convs1 = DWConv(ch, 256, 3, 1)
        self.iou_convs2 = DWConv(256, 128, 3, 1)
        self.iou_convs3 = DWConv(128, 64, 3, 1)
        self.obj_convs2 = DWConv(ch, 256, 3, 1)
        # self.obj_convs2 = XSepConv(ch, 256, 3, 1,padding=1)
        # 引入SimAM注意力机制
    ...
    def forward(self, x):
        # 首先通过共享的 Conv_DCNv4 层
        x = self.shared_conv_dcnv4(x)
...
class VGGAM_Block(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.simam = SimAM(e_lambda=1e-4)  # 添加 SimAM 注意力机制
        self.add = shortcut and c1 == c2
