...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, Focal=False, alpha=1, gamma=1.5, eps=1e-7, loss_weight=1.0):
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU
            ...
            if CIoU:
                ...
                if Focal:
                    return iou - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alpha)) * loss_weight, torch.pow(inter/(union + eps), gamma)  # Focal_CIoU
                else:
                    return iou - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alpha)) * loss_weight  # CIoU
            elif EIoU:
                ...
                if Focal:
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2) * loss_weight, torch.pow(inter/(union + eps), gamma) # Focal_EIoU
                else:
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2) * loss_weight # EIoU
            elif SIoU:
                # SIoU Loss
                ...
                if Focal:
                    return iou - torch.pow(0.5 * (distance_cost + shape_cost) + eps, alpha) * loss_weight, torch.pow(inter/(union + eps), gamma) # Focal_SIoU
                else:
                    return iou - torch.pow(0.5 * (distance_cost + shape_cost) + eps, alpha) * loss_weight # SIoU
            if Focal:
                return iou - rho2 / c2 * loss_weight, torch.pow(inter/(union + eps), gamma)  # Focal_DIoU
            else:
                return iou - rho2 / c2 * loss_weight  # DIoU
        c_area = cw * ch + eps  # convex area
        if Focal:
            return iou - torch.pow((c_area - union) / c_area + eps, alpha) * loss_weight, torch.pow(inter/(union + eps), gamma)  # Focal_GIoU
        else:
            return iou - torch.pow((c_area - union) / c_area + eps, alpha) * loss_weight  # GIoU

    if Focal:
        return iou, torch.pow(inter/(union + eps), gamma)  # Focal_IoU
    else:
        return iou * loss_weight  # IoU
