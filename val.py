...
def run(
    ...
    if save_json and len(jdict):
        ...
        anno_json = str(Path("instances_BDD100K.json"))  # annotations
        if not os.path.exists(anno_json):
            anno_json = os.path.join(data["path"], "annotations", "instances_BDD100K.json")
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "runs/train/exp214/weights/best.pt", help="model path(s)")
